...
if [ -z "${ONLY_TARGET}" ] && [ -z "${ONLY_COMMON}" ]; then
    ...
    setup_vendor "${DEVICE_PLATFORM}" "${VENDOR}" "${ANDROID_ROOT}" true "${CLEAN_VENDOR}"

    extract "${MY_DIR}/proprietary-files.txt" "${SRC}" "${KANG}" --section "${SECTION}"
fi

if [ -z "${ONLY_TARGET}" ] && [ -z "${ONLY_PLATFORM}" ] && [ -s "${MY_DIR}/../${DEVICE}/proprietary-files.txt" ]; then
...
if [ -z "${ONLY_COMMON}" ] && [ -z "${ONLY_PLATFORM}" ] && [ -s "${MY_DIR}/../${DEVICE}/proprietary-files.txt" ]; then
